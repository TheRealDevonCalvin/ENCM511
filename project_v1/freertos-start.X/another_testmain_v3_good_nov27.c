...
//    Disp2String("hello in main\n\r");
    ...
    for(;;){
        ...
        /* lowkey chat im now debating if this case statement shouldnt be like this but have the queue receive
         * in the main state machine so that theres less spaghetti-seeming logic????
         * something maybe i want to try when i have the kit again, but also do we care that much??
        */
        
        
        while(xQueueReceive(ProcessedButtonQueue, &pevt, 0) == pdTRUE){     // funny if statement? wowuld if be better?
            // receive events from the processed button queue, and switch on their type (click, long, etc.)
            switch(pevt.type){
                case EVT_CLICK:
                    // this case is entered if the type of button event from the processed event in the queue is a simple click
                    
                    // DEBUG PRINTS!!!! --------------------------------------------------
                    // the debug prints are NOT mutex protected rn, keep that in mind
                    ...
                    break;
                case EVT_LONG:
                    // this case is entered if the type of processed button event is a long click
                    ...
                    break;
                case EVT_COMBO:
                    // this case is entered when the processed event type was a combo click
                ...
                case EVT_LONG_COMBO:
                    // this case is entered if the type of processed event was a long combo click
        ...
        //----------------------------------------------------------------
        // is the 50ms wait fine?? do we want to wait??? i cant remember the rationale for this one chat
        //----------------------------------------------------------------
        
        
        if(xTaskNotifyWait(0x00, 0xFFFFFFFFUL, &ulNotifiedValue, pdMS_TO_TICKS(50)) == pdTRUE){
            // this basically receives notifications of events from some of the timers, in this application
            // the call clears no bits on entry, all bits on exit, pass the value into the ulNotifiedValue, and we wait 50ms of ticks
            if(ulNotifiedValue & (1<<NOTIFY_COUNTDOWN)){
                // if the notified value was the countdown end notification,
